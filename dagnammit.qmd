---
title: "DAG Nammit"
subtitle: "The Challenges and Dagners of Causally Interpreting Machine Learning Models"
...
- Current UC LCB adjunct :man_teacher: (~7 years)
...
::: {.column width="50%"}
![](images/logos.png){width="60%" fig-align=center}

![](images/books.png){width="60%" fig-align=center}
:::
...
::: {.column width="50%"}
[**Levels of explainability**]{style="color: forestgreen;"}

* Global
  - [Which variables seem "important?"](https://journal.r-project.org/archive/2020/RJ-2020-013/index.html) ([vip](https://cran.r-project.org/package=vip))
  - [How does each feature impact the model's predictions?](https://journal.r-project.org/archive/2017/RJ-2017-016/index.html) ([pdp](https://cran.r-project.org/package=pdp))
* Local
  - Explaining individual predictions ([fastshap](https://cran.r-project.org/package=fastshap))

:::

::: {.column width="50%"}
[**Methods of explainability**]{style="color: forestgreen;"}

* Model-specific
  - Model coefficients
  - Inspect tree structure :evergreen_tree:
* Model-agnostic 
  - Can be applied to any prediction model :warning:
...
## Black-box vs. [glass]{style="color: dodgerblue;"}-box models {.smaller}

:::: {.columns}

::: {.column width="50%"}
**Black**-box models :black_large_square::

* Often require (expensive) post-hoc techniques that require additional assumptions to explain
  - :eyes: Looking at you permutation methods! :smirk:
* Often misleading without proper constraints (e.g., interactions and monotonicity)
* IMO, [*multicollinearity*](https://en.wikipedia.org/wiki/Multicollinearity) is even more of a problem here!

:::

::: {.column width="50%"}
**[glass]{style="color: dodgerblue;"}**-box models :ice_cube::
...
* Natutrally interpretable structure
 ...
 - [Explainable boosting machines](https://interpret.ml/docs/ebm.html)!
   ![](images/iml-logo.png){width="60%" fig-align=center}
:::
...
- [*Explainable machine learning*]{style="color: cornflowerblue;"}: "use of post hoc methods to understand how a model makes predictions"

. . .

- [*Interpretable machine learning*]{style="color: cornflowerblue;"}: "the ability to explain or to present in understandable terms to a human."

. . .

- In essense: "[interpretability]{style="color: darkorange;"} > [explainability]{style="color: darkorange;"}"
...
![](images/ellie-tweet.png){fig-align=center}


## Correlation does not imply causation {.smaller}
...
::: {.callout-tip}
Checking [*variance inflation factor*](https://en.wikipedia.org/wiki/Variance_inflation_factor)s (VIFs) is always a good idea!
:::
...
corrplot::corrplot(cor(X), method = "ellipse", order = "AOE", type = "upper")
```
...
Variable importance scores from an [XGBoost](https://xgboost.readthedocs.io/en/stable/) fit:
...
## [Partial dependence (PD) plots](https://christophm.github.io/interpretable-ml-book/pdp.html)
...
## [DAGitty](https://www.dagitty.net/) (copy and paste)
...
## Retention example (cont.) {.smaller}
...
#| fig-width: 6
#| fig-asp: 0.618
autoplot(pds[[3L]]) + ylim(0, 1) +
    geom_line(data = pds.truth[[3]], color = "red") +
    ylim(0, 1) + 
    ylab("Partial dependence")
```
...
Using another XGBoost model, here's the estimated PD of `Did.renew` on `Ad.spend`, conditional on `Monthly.usage` and `Last.upgrade`: ... :drum:
...
Given a causal model, double ML *[essentially](https://docs.doubleml.org/stable/workflow/workflow.html)* involves three steps:

1) Predict the outcome ($y$) from an [appropriate adjustment set]{style="color: forestgreen;"} and get the residuals ($r_y$)

2) Predict the treatment ($x$) from an [appropriate adjustment set]{style="color: green;"} and get the residuals ($r_x$)

3) *Regress* $r_y$ on $r_x$ to create a model of the heterogeneous treatment effect
...
* Oftentimes multiple reasonable DAGs will exist 
  - [Sensitivity analysis](https://cran.r-project.org/package=causalsens)!
...
![](https://i.giphy.com/media/v1.Y2lkPTc5MGI3NjExY3QzZHJ0aWp6cTF0N2EzNGpkcHQwZW8xM3NlaGk5NjFoank2NjR1cyZlcD12MV9pbnRlcm5hbF9naWZfYnlfaWQmY3Q9Zw/SaKvh2fShKr1hYZssC/giphy.gif)
